...
|---|---|---|
| **Traffic intensity** | \( I = \frac{La}{R} \) | *L* = packet size [bits/pkt], *a* = arrival rate [pkts/s], *R* = tx rate [bits/s]. As *I* → 1, queuing delay → ∞. |
| **Transmission delay** | Node: \( d_{trans} = \frac{L}{R} \) · End-to-end: \( d_{trans} = \frac{NL}{R} \) | *N* = number of nodes |
...
| Model | Formula | Notes |
|---|---|---|
| **Client-server** | \( D_{cs} \geq \max\!\left\{\frac{NF}{u_s},\; \frac{F}{d_{min}}\right\} \) | *N* = peers, *F* = file size [bits], *u_s* = server upload [bits/s], *d_min* = min peer download. Scales **linearly** with *N*. (≥ often = ) |
| **Peer-to-peer** | \( D_{P2P} \geq \max\!\left\{\frac{F}{u_s},\; \frac{F}{d_{min}},\; \frac{NF}{u_s + \sum_{i=1}^{N} u_i}\right\} \) | *u_i* = upload rate of peer *i*. Server sends one copy; peers redistribute. Scales **much slower** than C/S. (≥ often = ) |
...
| **EstimatedRTT** | \( (1-\alpha)\cdot EstRTT + \alpha\cdot SampleRTT \) | EWMA; recommended α = 0.125 |
| **DevRTT** | \( (1-\beta)\cdot DevRTT + \beta\cdot\|SampleRTT - EstRTT\| \) | EWMA; recommended β = 0.25 |
| **Timeout interval** | \( EstRTT + 4\cdot DevRTT \) | Init = 1 s. Doubled after timeout; recalculated after ACK. |
